...
since sliced bread. They open up new frontiers of performance and
composition.
...
* Reducing Like It's 1979
  ...
  instead of getting biographical, let's have a crack at one of my
  favorite exercises in the book: implementing ~reduce~:
  ...
  In general, the function ~reduce~ takes has to have the following
  signature:
...
* Standing On The Shoulders Of Reducers
  ...
  like a problem, but it's there. If you cannot guess, it's the only
  concretion left in ~mapper~. What happens if we factor that one out?
  How do we treat it? What is the purpose of ~conj~?

  ~conj~ is a combiner, or an accumulating function. Let's factor it out
  as ~rf~:
  ...
  complicated. Suddenly ~conj~ looks more and more out of place. What
  happens if we pull it out even further?
  ...
  #+begin_src clojure
    ...
    (defn -map
      [f init coll]
      (fn [rf]
        (-reduce ((-mapper f) rf) init coll)))

    ((-map inc [] (range 3)) conj) ;; => [1 2 3]
    ((-map inc 0 (range 3)) +) ;; => 6
  ...
  completely separated the behavior of mapping over a sequence, from the
  behavior of accumulating the results of the mapping into something.
  ...
  with a loop, so everything you can think of that can be implemented by
  one of those, can turn into a function changes the behavior of a
  reducing process. Transforms it. A trans-ducer.
